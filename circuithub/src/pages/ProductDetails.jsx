...
// import { ShoppingCart, LogIn, UserPlus, Heart, Star } from "lucide-react";
// import { useAuth } from "../context/AuthContext";

// // Redesigned ProductDetails UI — functionality preserved, improved mobile preview/responsiveness.
...
//         { withCredentials: true }
//       );
//       setReviews((r) => [...r, { user: user.email || "You", comment: newReview, date: new Date().toLocaleDateString() }]);
//       setNewReview("");
//     } catch (err) {
...
//   const specsArray =
//     Array.isArray(product.specifications) ? product.specifications : typeof product.specifications === "string" ? product.specifications.split("\n").filter(Boolean) : [];
...
//             <h1 className="text-xl font-bold text-[#2D7D9A] cursor-pointer" onClick={() => navigate("/")}>CircuitHub</h1>
//             <div className="hidden sm:flex items-center gap-3 text-sm text-[#475569]">
//               <span className="px-2 py-1 rounded-md bg-white/60">{product?.brand || "Independent"}</span>
//               <span className="px-2 py-1 rounded-md bg-white/60">{product?.category}</span>
//             </div>
//           </div>

//           <div className="flex items-center gap-4">
//             <div className="hidden sm:flex items-center gap-2">
//               <button onClick={() => navigate("/auth")} className="flex items-center gap-2 text-sm">
//                 <LogIn /> Sign in
//               </button>
...
//               <ShoppingCart className="text-[#1E293B]" />
//               {cartCount > 0 && (
//                 <span className="absolute -top-2 -right-2 bg-[#F9A826] text-[#1E293B] text-xs font-bold rounded-full px-1.5">{cartCount}</span>
//               )}
//             </div>
//           </div>
//         </div>
...
//         {/* Top area: gallery + info + buy card */}
//         <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 items-start">
//           {/* Gallery */}
//           <div className="col-span-1">
//             <div
//               className="bg-white rounded-2xl border border-[#E2E8F0] p-3 sm:p-4 shadow-sm hover:shadow-lg transition relative overflow-hidden"
//               onMouseEnter={() => setHovered(true)}
//               onMouseLeave={() => setHovered(false)}
//             >
//               {/* mobile-friendly height: h-44 on small, lg:h-72 on large */}
//               <div className={`w-full h-44 lg:h-72 flex items-center justify-center transition-transform duration-300 ${hovered ? "transform scale-105" : ""}`}>
//                 <img src={images[mainIndex]} alt={product.name} className="max-h-full object-contain rounded-md" />
//               </div>
...
//                     className={`flex-shrink-0 w-16 h-16 rounded-md p-0.5 border ${i === mainIndex ? "border-[#2D7D9A]" : "border-transparent"} bg-white`}
//                   >
...
//             {/* compact product summary under gallery for mobile - clearer, bigger CTA */}
//             <div className="mt-4 lg:hidden bg-white border border-[#E2E8F0] rounded-lg p-3 shadow-sm">
//               <div className="flex items-center justify-between">
//                 <div>
//                   <div className="text-lg font-semibold text-[#1E293B]">${product.price}</div>
//                   <div className="text-xs text-[#475569]">Est. delivery: {product.delivery || "3-5 days"}</div>
//                 </div>
//                 <div className="flex items-center gap-2">
//                   <button onClick={handleAddToCart} className="px-3 py-2 rounded bg-[#F9A826] text-[#1E293B] text-sm">Add</button>
//                   <button onClick={handleBuyNow} className="px-3 py-2 rounded bg-[#2D7D9A] text-white text-sm">Buy</button>
//                 </div>
//               </div>

//               {/* qty controls collapsed for mobile — simple plus/minus inline */}
//               <div className="mt-3 flex items-center gap-2">
//                 <button onClick={() => changeQty(-1)} className="px-2 py-1 border rounded">-</button>
//                 <div className="px-3 py-1 border rounded">{qty}</div>
//                 <button onClick={() => changeQty(1)} className="px-2 py-1 border rounded">+</button>
//                 <button onClick={() => { if (!user) navigate('/auth'); else alert('Wishlist (demo)'); }} className="ml-auto p-2 border rounded bg-white"> <Heart /> </button>
//               </div>
//             </div>
//           </div>
...
//           {/* Info column */}
//           <div className="col-span-1">
//             <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#0F172A]">{product.name}</h2>

//             {/* mobile summary row: price + rating + stock (compact) */}
//             <div className="mt-2 flex items-center gap-3 text-sm text-[#475569]">
//               <div className="flex items-center gap-2"><Star className="text-yellow-500" /> <span>{product.rating ?? "-"}</span></div>
//               <div className="text-sm text-[#64748B]">• {product.reviews?.length ?? reviews.length} reviews</div>
//               <div className={`px-2 py-1 rounded text-sm ${product.stock > 0 ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>{product.stock > 0 ? "In Stock" : "Out of Stock"}</div>
//             </div>

//             <p className="mt-3 text-[#475569]">{product.shortDescription || product.description}</p>

//             <div className="mt-4 grid grid-cols-2 gap-2 text-sm text-[#475569]">
//               {product.sku && <div className="bg-white border rounded p-2">SKU<br/><strong className="text-[#1E293B]">{product.sku}</strong></div>}
//               {product.brand && <div className="bg-white border rounded p-2">Brand<br/><strong className="text-[#1E293B]">{product.brand}</strong></div>}
//               {product.weight && <div className="bg-white border rounded p-2">Weight<br/><strong className="text-[#1E293B]">{product.weight}</strong></div>}
//               {product.dimensions && <div className="bg-white border rounded p-2">Dimensions<br/><strong className="text-[#1E293B]">{product.dimensions}</strong></div>}
//             </div>

//             {/* Specifications - full panel on lg+, collapsible details on mobile */}
//             <div className="mt-4">
//               <div className="hidden lg:block bg-[#F1F5F9] p-4 rounded-lg">
//                 <h3 className="font-semibold text-lg text-[#1E293B] mb-2">Specifications</h3>
//                 <ul className="text-[#475569] list-disc list-inside text-sm">
//                   {specsArray.length ? specsArray.map((s, idx) => <li key={idx}>{s}</li>) : <li>No specifications provided.</li>}
//                 </ul>
//               </div>

//               <details className="lg:hidden bg-white border border-[#E2E8F0] rounded p-3"> 
...
//               <div className="bg-white border border-[#E2E8F0] rounded-xl p-5 shadow">
//                 <div className="flex items-start justify-between gap-4">
//                   <div>
//                     <div className="text-sm text-[#475569]">Price</div>
//                     <div className="text-2xl font-bold text-[#1E293B]">${product.price}</div>
//                     {product.mrp && <div className="text-xs line-through text-[#64748B]">${product.mrp}</div>}
//                   </div>
//                   <div className="text-right text-sm text-[#16A34A]">{product.mrp ? `Save $${Math.max(0, (product.mrp - product.price).toFixed(2))}` : ""}</div>
//                 </div>

//                 <div className="mt-4 flex items-center gap-3">
//                   <button onClick={() => changeQty(-1)} className="px-3 py-2 border rounded">-</button>
//                   <div className="px-3 py-2 border rounded min-w-[40px] text-center">{qty}</div>
//                   <button onClick={() => changeQty(1)} className="px-3 py-2 border rounded">+</button>
//                 </div>

//                 <div className="mt-4 grid grid-cols-2 gap-3">
//                   <button onClick={handleAddToCart} className="py-3 rounded bg-[#F9A826] text-[#1E293B] font-semibold">Add to Cart</button>
//                   <button onClick={handleBuyNow} className="py-3 rounded bg-[#2D7D9A] text-white font-semibold">Buy Now</button>
//                 </div>

//                 <div className="mt-4 flex items-center gap-3">
//                   <button
//                     onClick={() => { if (!user) navigate("/auth"); else alert("Added to wishlist (demo)"); }}
//                     className="flex items-center gap-2 px-3 py-2 border rounded bg-white"
//                   >
//                     <Heart size={16} /> Wishlist
//                   </button>
//                   <div className="text-xs text-[#64748B] ml-auto">Est. delivery: {product.delivery || "3-5 days"}</div>
//                 </div>
//               </div>
//             </div>

//             {/* Mobile sticky buybar (bigger targets, more legible) */}
//             <div className="lg:hidden fixed bottom-0 left-0 right-0 p-3 bg-white border-t border-[#E2E8F0] z-50">
//               <div className="max-w-7xl mx-auto px-2 flex items-center gap-3">
//                 <button onClick={() => { if (!user) navigate("/auth"); else alert("Wishlist (demo)"); }} className="p-2 rounded bg-white border">
//                   <Heart />
//                 </button>

//                 <div className="flex-1">
//                   <div className="text-sm text-[#1E293B] font-semibold">${product.price}</div>
//                   <div className="text-xs text-[#64748B]">Est. delivery: {product.delivery || "3-5 days"}</div>
//                 </div>

//                 <div className="flex gap-2">
//                   <button onClick={handleAddToCart} className="px-4 py-3 bg-[#F9A826] text-[#1E293B] rounded font-semibold">Add</button>
//                   <button onClick={handleBuyNow} className="px-4 py-3 bg-[#2D7D9A] text-white rounded font-semibold">Buy</button>
//                 </div>
//               </div>
//             </div>
//           </div>
...
//         {/* CAROUSELS: Similar / Related */}
//         <div className="mt-8">
//           <h3 className="text-lg sm:text-xl font-semibold text-[#1E293B] mb-3">Similar products</h3>
//           <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
//             {similar.length === 0 ? (
//               <div className="text-[#64748B]">No similar products found.</div>
//             ) : (
//               similar.map((s) => {
//                 const sId = getId(s);
//                 return (
//                   <div key={sId || s.name} className="bg-white border rounded-lg p-3 flex flex-col">
//                     <div onClick={() => navigate(`/product/${sId}`)} className="h-24 sm:h-28 flex items-center justify-center mb-2 cursor-pointer">
//                       <img src={s.images?.[0] || s.image || "https://via.placeholder.com/150"} alt={s.name} className="object-contain h-full" />
//                     </div>
//                     <div className="text-sm font-medium text-[#1E293B] line-clamp-2">{s.name}</div>
//                     <div className="text-sm text-[#64748B] mt-1">${s.price}</div>
//                     <div className="mt-2 flex gap-2">
//                       <button onClick={() => { if (!user) navigate("/auth"); else { axios.post("http://localhost:5000/api/cart/add", { productId: sId, quantity: 1 }, { withCredentials: true }).then(() => setCartCount((c) => c + 1)).catch((err) => console.debug("Similar add failed:", err?.message ?? err)); } }} className="flex-1 bg-[#F9A826] text-[#1E293B] py-2 rounded text-xs">Add</button>
//                       <button onClick={() => navigate(`/product/${sId}`)} className="flex-1 bg-[#2D7D9A] text-white py-2 rounded text-xs">View</button>
//                     </div>
//                   </div>
//                 );
//               })
...
//         <div className="mt-6">
//           <h3 className="text-lg sm:text-xl font-semibold text-[#1E293B] mb-3">Related products</h3>
//           <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
//             {related.length === 0 ? (
//               <div className="text-[#64748B]">No related products found.</div>
//             ) : (
//               related.map((r) => {
//                 const rId = getId(r);
//                 return (
//                   <div key={rId || r.name} className="bg-white border rounded-lg p-3 flex flex-col">
//                     <div onClick={() => navigate(`/product/${rId}`)} className="h-24 sm:h-28 flex items-center justify-center mb-2 cursor-pointer">
//                       <img src={r.images?.[0] || r.image || "https://via.placeholder.com/150"} alt={r.name} className="object-contain h-full" />
//                     </div>
//                     <div className="text-sm font-medium text-[#1E293B] line-clamp-2">{r.name}</div>
//                     <div className="text-sm text-[#64748B] mt-1">${r.price}</div>
//                     <div className="mt-2 flex gap-2">
//                       <button onClick={() => { if (!user) navigate("/auth"); else { axios.post("http://localhost:5000/api/cart/add", { productId: rId, quantity: 1 }, { withCredentials: true }).then(() => setCartCount((c) => c + 1)).catch((err) => console.debug("Related add failed:", err?.message ?? err)); } }} className="flex-1 bg-[#F9A826] text-[#1E293B] py-2 rounded text-xs">Add</button>
//                       <button onClick={() => navigate(`/product/${rId}`)} className="flex-1 bg-[#2D7D9A] text-white py-2 rounded text-xs">View</button>
//                     </div>
//                   </div>
//                 );
//               })
...
//                 <textarea rows={4} className="border rounded px-3 py-2 resize-none" placeholder="Share your experience..." value={newReview} onChange={(e) => setNewReview(e.target.value)} required />
//                 <button type="submit" className="bg-[#2D7D9A] text-white py-2 rounded hover:bg-[#3BA8C8] transition">Submit Review</button>
//               </form>
//             </div>
//           </div>
//         </div>
...
//   // NEW: control mobile review modal
...
//             <div>
//               {/* Desktop: direct textarea. Mobile: shows a small read-only textarea that opens modal */}
//               <form onSubmit={handleSubmitReview} className="flex flex-col gap-2">
//                 <label className="text-sm font-medium">Write a review</label>

//                 {/* Desktop textarea (visible on lg+) */}
//                 <textarea
//                   rows={4}
//                   className="border rounded px-3 py-2 resize-none hidden lg:block"
...
//                 {/* Mobile trigger (read-only) */}
//                 <textarea
//                   rows={2}
//                   className="border rounded px-3 py-2 resize-none lg:hidden"
...
//                 <button type="submit" className="hidden lg:inline-block bg-[#2D7D9A] text-white py-2 rounded hover:bg-[#3BA8C8] transition">Submit Review</button>
//               </form>
//             </div>
//           </div>
//         </div>

//         {/* Mobile review modal with blurred backdrop */}
//         {reviewModalOpen && (
//           <div className="fixed inset-0 z-50 flex items-center justify-center">
//             {/* Backdrop: blur + translucent dark layer */}
//             <div
//               className="absolute inset-0 backdrop-blur-md bg-black/40"
//               onClick={() => setReviewModalOpen(false)}
//             />

//             <div className="relative w-[92%] max-w-md bg-white rounded-xl p-4 shadow-2xl z-10">
//               <div className="flex items-center justify-between mb-2">
//                 <h3 className="text-lg font-semibold">Write a review</h3>
//                 <button onClick={() => setReviewModalOpen(false)} className="text-gray-500">✕</button>
//               </div>
...
//                 <div className="flex gap-2">
//                   <button type="button" onClick={() => setReviewModalOpen(false)} className="flex-1 py-2 rounded border">Cancel</button>
//                   <button type="submit" className="flex-1 py-2 rounded bg-[#2D7D9A] text-white">Submit</button>
//                 </div>
//               </form>
//             </div>
//           </div>
...
import { useParams, useNavigate } from "react-router-dom";
import axios from "axios";
import { ShoppingCart, LogIn, UserPlus, Heart, Star, Menu, ChevronLeft, ChevronRight } from "lucide-react";
import { useAuth } from "../context/AuthContext";
